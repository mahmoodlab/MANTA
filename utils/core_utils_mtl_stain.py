...
import os
import sys
from datasets.dataset_mtl import save_splits
...
def train(datasets, cur, args):
    ...
    print('\nInit Loaders...', end=' ')
    train_loader = get_split_loader(train_split, training=True, testing = args.testing, weighted = args.weighted_sample, collate_fn='MIL_mtl_stain')
    val_loader = get_split_loader(val_split, collate_fn='MIL_mtl_stain')
    test_loader = get_split_loader(test_split, collate_fn='MIL_mtl_stain')
    print('Done!')
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):
    ...
    for batch_idx, (data_HnE, data_Jon, data_Tri, data_PAS, label_task1, label_task2, label_task3) in enumerate(loader):
        ...
        results_dict = model(HnE=data_HnE, Jon=data_Jon, Tri=data_Tri, PAS=data_PAS, return_features=False)
        ...
        if (batch_idx + 1) % 5 == 0:
            print('batch {}, task1 loss: {:.4f}, task2 loss: {:.4f} task3 loss: {:.4f} '.format(batch_idx, loss_value_task1, loss_value_task2, loss_value_task3) +
                'label_task1: {}, label_task2: {}, label_task3: {}, '.format(label_task1.item(), label_task2.item(), label_task3.item()) + 
                ' bag_size (HnE): {}, bag_size (Jon): {}, bag_size (Tri): {}, bag_size (PAS): {}'.format(data_HnE.size()[0], data_Jon.size()[0], data_Tri.size()[0], data_PAS.size()[0]))
