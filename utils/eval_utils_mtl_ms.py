...
import torch
from models.model_attention_mil import MIL_Attention_fc_mtl_ms
import os
import pandas as pd
from utils.utils import *
from utils.core_utils_mtl_stain import Accuracy_Logger
...
def initiate_model(args, ckpt_path=None):
    print('Init Model')
    model_dict = {"dropout": args.drop_out, 'n_classes': args.n_classes, 'fusion': args.fusion}
    ...
    if args.model_type == 'attention_mil':
        model = MIL_Attention_fc_mtl_ms(**model_dict)
    else: 
...
def eval_(dataset, args, ckpt_path):
    ...
    print('Init Loaders')
    loader = get_simple_loader(dataset, collate_fn='MIL_mtl_stain')
...
def summary(model, loader, args):
    ...
    pbar = tqdm(loader, total=len(loader), initial=0, desc="Running Eval (MTL-MS)")

    bag_sizes = []

    for batch_idx, (data_HnE, data_Jon, data_Tri, data_PAS, label_task1, label_task2, label_task3) in enumerate(pbar):
        data_HnE     = data_HnE.to(device)
        data_Jon     = data_Jon.to(device)
        data_Tri     = data_Tri.to(device)
        data_PAS     = data_PAS.to(device)
        label_task1 = label_task1.to(device)
        label_task2 = label_task2.to(device)
        label_task3 = label_task3.to(device)
        bag_size = data_HnE.shape[0]+data_Jon.shape[0]+data_Tri.shape[0]+data_PAS.shape[0]
        bag_sizes.append(bag_size)

        case_id = case_ids.iloc[batch_idx]
        pbar.set_description('Running Eval (MTL-MS) - %s' % case_id)

        with torch.no_grad():
            model_results_dict = model(HnE=data_HnE, Jon=data_Jon, Tri=data_Tri, PAS=data_PAS, return_features=False)
        ...
        patient_results.update({case_id: {'case_id': np.array(case_id),
                                'prob_task1': probs_task1, 'label_task1': label_task1.item(),#.detach().cpu().numpy(),
                                'prob_task2': probs_task2, 'label_task2': label_task2.item(),#.detach().cpu().numpy(),
                                'prob_task3': probs_task3, 'label_task3': label_task3.item(),#.detach().cpu().numpy(),
                                'A': A_patient }})
