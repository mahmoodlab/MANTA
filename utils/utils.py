...
def collate_MIL_mtl_stain(batch):
	he = torch.cat([item[0] for item in batch], dim = 0)
	jones = torch.cat([item[1] for item in batch], dim = 0)
	trichrome = torch.cat([item[2] for item in batch], dim = 0)
	pas = torch.cat([item[3] for item in batch], dim = 0)
	label_task1 = torch.LongTensor([item[4] for item in batch])
	label_task2 = torch.LongTensor([item[5] for item in batch])
	label_task3 = torch.LongTensor([item[6] for item in batch])
	return [he, jones, trichrome, pas, label_task1, label_task2, label_task3]
...
collate_dict = {'MIL': collate_MIL, 'MIL_mtl': collate_MIL_mtl, 'MIL_mtl_stain': collate_MIL_mtl_stain, 'MIL_mtl_sex': collate_MIL_mtl_sex, 'MIL_sex': collate_MIL_mtl}
